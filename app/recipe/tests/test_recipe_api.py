...
class PrivateRecipeAPITests(TestCase):
    ...
    def test_create_recipe_with_new_ingredients(self):
        ...
        payload = {
            'title': 'Tacos with beans',
            'price': Decimal('19.99'),
            'time_minutes': 40,
            'ingredients': [{'name': 'Beans'}, {'name': 'Tacos'}],
        }
        res = self.client.post(RECIPES_URL, payload, format='json')
    ...
    def test_create_recipe_with_existing_ingredient(self):
        """Test create recipe with an existing ingredient"""
    ...
    def test_update_recipe_assign_ingredient(self):
        ...
        payload = {'ingredients': [{'name': 'Green pepper'}]}
        url = detail_url(recipe.id)
        res = self.client.patch(url, payload, format='json')
    ...
    def test_clear_recipe_ingredients(self):
        """Test clear ingredients from a recipe"""
    ...
    def test_filter_by_tags(self):
        ...
        recipe1 = create_recipe(user=self.user, title='Cauliflower curry')
        recipe2 = create_recipe(user=self.user, title='Zeekral salade')
        tag1 = Tag.objects.create(user=self.user, name='Vegan')
        tag2 = Tag.objects.create(user=self.user, name='Dinner')
        recipe1.tags.add(tag1)
        recipe2.tags.add(tag2)
        recipe3 = create_recipe(user=self.user, title='Baked paprika')
    ...
    def test_filter_by_ingredients(self):
        ...
        recipe1 = create_recipe(user=self.user, title='Red curry')
        recipe2 = create_recipe(user=self.user, title='Pasta al pomodoro')
        ing1 = Ingredient.objects.create(user=self.user, name='Tomatoes')
        ing2= Ingredient.objects.create(user=self.user, name='Red curry paste')
        recipe1.ingredients.add(ing2)
        recipe2.ingredients.add(ing1)
        recipe3 = create_recipe(user=self.user, title='Lentil golden soup')
...
class ImageUploadTests(TestCase):
    ...
    def test_upload_image(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.jpg') as image_file:
            ...
            # Save moves pointer to the end of the file.
            image_file.seek(0)
